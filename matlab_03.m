...
% 常见数学函数
% 自变量为矩阵，运算法则是将函数逐项作用在矩阵元素上
x_sin = 0:pi/2:2*pi;
x_sqrt = [1, 4, 9, 16, 25];
y = sin(x_sin);             % 三角函数：sin、cos、tan
y_asin = asin(x_sin);       % 反三角函数：asin、acos、atan
y_abs = abs(y);             % abs函数：求实数的绝对值、复数的模、字符串的ASCII码
y_sqrt = sqrt(x_sqrt);      % sqrt函数求平方根

% log、log2、log10、power等省略演示
% 上述分别表示以e、2、10为底的对数函数，求幂函数
...
% 比较矩阵时，对应元素比较并返回由0和1构成的新的同维度矩阵
a2 = [1, 2, 3; 4, 5, 6];
a3 = [3, 1, 4; 5, 2, 10];
result2 = a2>a3;

% 标量和矩阵比较，标量依次和矩阵元素比较，最后返回一个和矩阵同维度的新矩阵
result3 = a3>4;


% 逻辑运算
% &     与：a和b全非零时，结果为1；否则为0
% |     或：a和b中只要有一个非零，结果就为1；否则为0
% ~     非：a非零时，结果为0；a是零时，结果为1
% xor   异或：a和b值不同时，结果为1；相同时为0

% 获取向量整体情况的函数
% any   若向量的任一元素非零，any函数返回1；否则为0
% all   若向量的所有元素非零，all函数返回1；否则为0
